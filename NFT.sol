...
contract NFT is ERC721Enumerable, Ownable {
    ...
    constructor(
        address _addr_contr,
        address _addr_NFT_owner,
        address _addr_NFT_buyer
    ) ERC721("V", "V") {
        require(msg.sender == _addr_NFT_owner);
        token = YourToken(_addr_contr);
        setOwner(msg.sender);
        addr_NFT_buyer = _addr_NFT_buyer;
    }
    ...
    function buyTokens(uint256 amountToBuy) public returns (uint256 tokenAmount) {
        ...
        console.log(allCost);
        require(allCost <= token.balanceOf(addr_NFT_buyer), "Not enough balance of ERC20");

        token.transferFrom(addr_NFT_buyer, addr_NFT_owner, allCost);
        mint(amountToBuy);
    ...
    function mint(uint256 _mintAmount) public {
        ...
        if (addr_NFT_buyer != owner()) {
            require(token.balanceOf(addr_NFT_buyer) >= cost * _mintAmount);
        }
